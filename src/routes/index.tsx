    ...
    <AppLayout>
      ...
      <section className="mt-10">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-muted-foreground">Click to start planning</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {POPULAR_CITIES.slice(0, 6).map((c) => (
            <div key={c.name} onClick={() => navigate({ to: '/create' })} className="relative rounded-2xl overflow-hidden aspect-[3/4] hover-lift cursor-pointer group">
              ...
              <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/20 transition-colors duration-300 flex items-center justify-center">
                <span className="opacity-0 group-hover:opacity-100 transition-opacity text-white text-xs font-bold bg-primary/80 px-2 py-1 rounded-full">Plan Trip →</span>
              </div>
